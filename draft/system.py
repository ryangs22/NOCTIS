...
class SistemaNoctis:
    def __init__(self):
        self.usuarios = []
        self.jogos_catalogo = []   # banco simulado de jogos (API Mock)
        self.trending = []
    ...
    def buscar_jogos(self, termo=""):
        ...
        resultados = [j for j in self.jogos_catalogo if termo.lower() in j.nome.lower() or termo.lower() in j.genero.lower()]
        print(f"Resultados para '{termo}': {len(resultados)} jogo(s) encontrado(s).")
        return resultados

    # Funcionalidade 3
    def marcar_status(self, usuario, jogo, status):
        for item in usuario.biblioteca:
            if item["jogo"] == jogo:
                item["status"] = status
                print(f"Status alterado: {jogo.nome} → {status}")
                return
        usuario.biblioteca.append({"jogo": jogo, "status": status})
    ...
    # Funcionalidade 6
    def criar_conquista(self, nome, descricao, jogo=None):
        conquista = Conquista(nome, descricao, jogo)
    ...
    def mostrar_perfil(self, usuario):
        ...
        print(f"Conquistas: {len(usuario.conquistas_desbloqueadas)}")
        print(f"Listas personalizadas: {len(usuario.listas_personalizadas)}")
        if usuario.listas_personalizadas:
            print("Listas:", [l.nome_lista for l in usuario.listas_personalizadas])

    # Funcionalidade 9
    def atualizar_trending(self):
        self.trending = sorted(self.jogos_catalogo, key=lambda j: len(j.avaliacoes), reverse=True)[:5]
    ...
    def mostrar_detalhes_jogo(self, jogo):
        ...
        print(f"Média de avaliações: {jogo.media_avaliacoes()}★")
        if jogo.avaliacoes:
